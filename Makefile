...
build-image:
	time docker build $(DOCKER_BUILD_FLAGS) \
		--tag $(PUBLIC_DOCKER_REGISTRY)/$(DOCKER_PROJECT)/terminator:$(IMAGE_TAG) \
		--tag $(PUBLIC_DOCKER_REGISTRY)/$(DOCKER_PROJECT)/terminator:$(IMAGE_ANCHOR_TAG) \
		.
push-image:
	docker push $(PUBLIC_DOCKER_REGISTRY)/$(DOCKER_PROJECT)/terminator:$(IMAGE_ANCHOR_TAG)
